...
import shutil 
fh1 = open('/face_data/LabelMeAnnotationTool/annotationCache/DirLists/labelme.txt','r')
...
while(k<len(img_list)):
    ...
    imagename=imname
    imname = imname[:-4]
    annotatedfilepath='/face_data/LabelMeAnnotationTool/Annotations/new/'+imname+'.xml'
    ...
    def gen_coordinate_images_copy(annotatedfilepath,image_name,subdirectorypath):
            #directory to save the images 
             imagesavedir= '/face_data/LabelMeAnnotationTool/Images/'+subdirectorypath
             ...
             for idx,rootobject in enumerate(root.findall("object")):
                 ...
                 for bndboxobj in rootobject.findall('bndbox'):   
                     for xmin,xmax,ymin,ymax in zip(bndboxobj.iter('xmin'),bndboxobj.iter('xmax'),bndboxobj.iter('ymin'),bndboxobj.iter('ymax')):
                         shutil.copyfile(image,imagesavedir+'/'+ image_name+'_'+str(idx)+'_'+xmin.text+'_'+ymin.text+'_'+xmax.text+'_'+ymax.text+'.jpg')
    def gen_xml():
        ...
        xml = xml.etree.ElementTree.ElementTree(file='/face_data/LabelMeAnnotationTool/Annotations/example_folder/'+imname+'.xml')
